...
pthread_rwlock_t readerwriter_mutex;
...
void list_init(list_node_t **head)
{
  *head = (list_node_t *)malloc(sizeof(list_node_t));
  ...
  pthread_rwlock_init(&readerwriter_mutex, NULL);
}
...
int list_add(list_node_t *head, int item)
{
  ...
  pthread_rwlock_wrlock(&readerwriter_mutex);	// Lock
  ...
  pthread_rwlock_unlock(&readerwriter_mutex);	// Unlock
...
int list_contain(list_node_t *head, int item)
{
  ...
  pthread_rwlock_rdlock(&readerwriter_mutex);	// Lock
  curr = head->next;
  while (curr->key < item) {
    curr = curr->next;
  }
